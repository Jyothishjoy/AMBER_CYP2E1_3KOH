...
19. Prepare MCPB.in file:

                original_pdb 3KOH_amber.pdb
               group_name 3KOH
               cut_off 3.0
               ion_ids 7774
               ion_mol2files FE.mol2
               naa_mol2files HEM.mol2 sub.mol2 OX.mol2 
...
Fix the FE "ion_ids" to match the Fe-atom number in your pdb file. 
Fix the Charge and multiplicity for the model. CPD-I ground state is quartet spin state. From the model with HEME_(COO-)2 (-4 charge), O(-2 charge) and SH (-1 charge) and Fe(+5 charge) = -2 overall charge.
I assumed +5 charge for Fe because we know from our CPD-I modeling that without the COO- functionalities, the system should be neutral and quartet. 

20. Now, run MCPB.py to generate large and small model Gaussian input files.

            source mcpb1.sh

This will generate three Gaussian input files and a bunch of fingerprint files. Double-check connections in “3KOH_standard.fingerprint” file. It should show that Amber considers Fe --> M1, S --> Y1, OX --> Y2, and four connecting N to Fe as Y3-Y5. At the end of the file, it should also print FE connectivity information. Make sure that Fe has six coordinations. In the final step, we need to cross-check this information with the prmtop and inpcrd files generated by MCPB.py

21. Edit the Gaussian com files to adjust the basis sets 6-31G*-SDD. See attached Gaussian com files for their specific format.
